#!/usr/bin/env python3
"""
Test script to verify comprehensive environmental data in daily briefing
"""

import asyncio
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), 'backend'))

from services.llm_service import LLMService

async def test_comprehensive_briefing():
    """Test the comprehensive environmental data briefing functionality"""
    
    # Initialize LLM service
    llm_service = LLMService()
    
    # Test user context with specific profile
    user_context = {
        "name": "Sarah",
        "allergies": ["pollen", "dust mites", "pet dander"],
        "asthma_severity": "severe",
        "triggers": ["smoke", "cold air", "exercise"],
        "age": 35,
        "household_info": {"pets": "2 cats", "smoking": "none"}
    }
    
    # Test risk prediction
    risk_prediction = {
        "risk_level": "moderate",
        "risk_score": 65,
        "factors": {
            "pollen": "high",
            "air_quality": "moderate",
            "temperature": "cold"
        },
        "recommendations": [
            "Avoid outdoor exercise",
            "Use air purifier",
            "Keep rescue inhaler handy"
        ]
    }
    
    # COMPREHENSIVE ENVIRONMENTAL DATA - ALL PARAMETERS
    environmental_data = {
        "aqi": 85,
        "pm25": 35.2,
        "pm10": 45.8,
        "ozone": 120.5,
        "temperature_c": 12,
        "humidity_pct": 65,
        "pollen": {
            "tree": 8.5,
            "grass": 6.2,
            "weed": 4.1,
            "overall_risk": "high"
        },
        "fires": {
            "count_100km": 3,
            "risk_level": "moderate"
        },
        "solar": {
            "storm_level": 2,
            "kp_index": 4.5
        },
        "precipitation": {
            "intensity": 2.5,
            "probability": 75
        },
        "wind": {
            "speed": 15.2,
            "direction": 180
        },
        "uv": {
            "index": 6,
            "level": "high"
        }
    }
    
    user_context["environmental"] = environmental_data
    
    print("Testing Comprehensive Environmental Data Briefing")
    print("=" * 60)
    print(f"User Profile: {user_context['name']}")
    print(f"Allergies: {user_context['allergies']}")
    print(f"Asthma Severity: {user_context['asthma_severity']}")
    print(f"Known Triggers: {user_context['triggers']}")
    print(f"Risk Level: {risk_prediction['risk_level']} ({risk_prediction['risk_score']}/100)")
    print()
    
    print("COMPREHENSIVE ENVIRONMENTAL DATA:")
    print(f"  Air Quality: AQI {environmental_data['aqi']}, PM2.5 {environmental_data['pm25']} μg/m³, PM10 {environmental_data['pm10']} μg/m³")
    print(f"  Ozone: {environmental_data['ozone']} μg/m³")
    print(f"  Temperature: {environmental_data['temperature_c']}°C, Humidity: {environmental_data['humidity_pct']}%")
    print(f"  Pollen: Tree {environmental_data['pollen']['tree']}, Grass {environmental_data['pollen']['grass']}, Weed {environmental_data['pollen']['weed']}")
    print(f"  Forest Fires: {environmental_data['fires']['count_100km']} fires within 100km, Risk: {environmental_data['fires']['risk_level']}")
    print(f"  Solar Activity: Storm Level {environmental_data['solar']['storm_level']}, KP Index {environmental_data['solar']['kp_index']}")
    print(f"  Precipitation: {environmental_data['precipitation']['intensity']} mm/h, Probability: {environmental_data['precipitation']['probability']}%")
    print(f"  Wind: {environmental_data['wind']['speed']} m/s, Direction: {environmental_data['wind']['direction']}°")
    print(f"  UV Index: {environmental_data['uv']['index']} ({environmental_data['uv']['level']})")
    print()
    
    try:
        # Test the briefing generation
        briefing = await llm_service.generate_daily_briefing(risk_prediction, user_context)
        
        print("Generated Comprehensive Briefing:")
        print("-" * 40)
        print(briefing)
        print()
        
        # Check if briefing includes comprehensive data
        name_mentioned = user_context["name"] in briefing
        allergies_mentioned = any(allergy in briefing.lower() for allergy in user_context["allergies"])
        severity_mentioned = user_context["asthma_severity"] in briefing.lower()
        triggers_mentioned = any(trigger in briefing.lower() for trigger in user_context["triggers"])
        
        # Check for environmental data mentions
        aqi_mentioned = str(environmental_data["aqi"]) in briefing
        pm25_mentioned = str(environmental_data["pm25"]) in briefing
        pollen_mentioned = str(environmental_data["pollen"]["tree"]) in briefing
        fires_mentioned = str(environmental_data["fires"]["count_100km"]) in briefing
        solar_mentioned = str(environmental_data["solar"]["kp_index"]) in briefing
        precipitation_mentioned = str(environmental_data["precipitation"]["intensity"]) in briefing
        wind_mentioned = str(environmental_data["wind"]["speed"]) in briefing
        uv_mentioned = str(environmental_data["uv"]["index"]) in briefing
        
        print("Comprehensive Data Check:")
        print(f"✓ Name mentioned: {name_mentioned}")
        print(f"✓ Allergies referenced: {allergies_mentioned}")
        print(f"✓ Asthma severity mentioned: {severity_mentioned}")
        print(f"✓ Triggers referenced: {triggers_mentioned}")
        print()
        print("Environmental Data Integration:")
        print(f"✓ AQI data: {aqi_mentioned}")
        print(f"✓ PM2.5 data: {pm25_mentioned}")
        print(f"✓ Pollen data: {pollen_mentioned}")
        print(f"✓ Fire data: {fires_mentioned}")
        print(f"✓ Solar data: {solar_mentioned}")
        print(f"✓ Precipitation data: {precipitation_mentioned}")
        print(f"✓ Wind data: {wind_mentioned}")
        print(f"✓ UV data: {uv_mentioned}")
        
        total_checks = 12
        passed_checks = sum([
            name_mentioned, allergies_mentioned, severity_mentioned, triggers_mentioned,
            aqi_mentioned, pm25_mentioned, pollen_mentioned, fires_mentioned,
            solar_mentioned, precipitation_mentioned, wind_mentioned, uv_mentioned
        ])
        
        print(f"\nOverall Score: {passed_checks}/{total_checks} checks passed")
        
        if passed_checks >= 8:
            print("✅ SUCCESS: Comprehensive briefing is working well!")
        elif passed_checks >= 6:
            print("⚠️  PARTIAL: Briefing includes most data but could be improved")
        else:
            print("❌ WARNING: Briefing may not be using comprehensive data effectively")
            
    except Exception as e:
        print(f"Error testing briefing: {e}")
        print("This might be expected if AI services are not available")

if __name__ == "__main__":
    asyncio.run(test_comprehensive_briefing())
