#!/usr/bin/env python3
"""
Test script to verify real-time briefing functionality for voice commands
"""
...
async def test_realtime_briefing():
    """Test the real-time briefing functionality for voice commands"""
    ...
    print("Testing Real-Time Briefing for Voice Commands")
    print("=" * 60)
    ...
    try:
        # Test the real-time briefing generation
        briefing = await llm_service.generate_real_time_briefing(risk_prediction, user_context)
        
        print("Generated Real-Time Briefing:")
        print("-" * 50)
        print(briefing)
        print()
        
        # Check if briefing includes real-time features
        ...
        real_time_greeting = "current" in briefing.lower() or "right now" in briefing.lower()
        real_time_analysis = "real-time" in briefing.lower() or "current" in briefing.lower()
        voice_optimized = len(briefing) < 2000  # Should be concise for voice delivery
        
        print("Real-Time Briefing Features Check:")
        ...
        print(f"✓ Real-time greeting: {real_time_greeting}")
        print(f"✓ Real-time analysis: {real_time_analysis}")
        print(f"✓ Voice optimized: {voice_optimized}")
        ...
        passed_checks = sum([
            ...
            real_time_greeting, real_time_analysis, voice_optimized
        ])
        
        print(f"\nOverall Score: {passed_checks}/{total_checks} checks passed")
        
        if passed_checks >= 13:
            print("✅ SUCCESS: Real-time briefing is working excellently for voice commands!")
        elif passed_checks >= 11:
            print("⚠️  GOOD: Real-time briefing is working well but could be improved")
        elif passed_checks >= 9:
            print("⚠️  PARTIAL: Some real-time features included but needs work")
        else:
            print("❌ WARNING: Real-time briefing may not be working effectively")
            
    except Exception as e:
        print(f"Error testing real-time briefing: {e}")
        print("This might be expected if AI services are not available")

if __name__ == "__main__":
    asyncio.run(test_realtime_briefing())
